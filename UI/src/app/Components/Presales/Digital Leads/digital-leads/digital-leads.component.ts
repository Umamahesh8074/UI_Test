...
@Component({
  selector: 'app-digital-leads',
  templateUrl: './digital-leads.component.html',
  styleUrls: ['./digital-leads.component.css']
})
export class DigitalLeadsComponent {
...
campagin = new FormControl({ campaginName: '' }); // Default as an object
  ...
  private initializeState() {
    ...
    this.digitalPartnerName = state.digitalPartner!=undefined?state.digitalPartner:this.digitalPartnerName;
    const camp={'campaginName':state.campaginName}
    this.campagin.patchValue(camp)
    console.log(this.campagin)
    this.campaginName=state.campaginName?state.campaginName:''
...
this.showLoading()
    this.leadService
      .getDashBoardLeadsDetails(
        ...
        this.preSalesId?this.preSalesId:'',
      ...
      .subscribe({
        next: (response:any) => {
          if(this.isExportExcel){
            this.downloadXLSFile(response, 'Leads '+Date.now().toString(), false);
          }
          else {this.leads = response.records;
  ...
  getUserFormLocalStorage() {
    ...
    if (user) {
      ...
      if (this.user.roleName.includes('DIGITAL_PARTNER')) {
  ...
  goBack() {
    ...
    const stateData = {
      ...
      camapaginName:this.campaginName,
      sourceIds:this.sourceId,
...
this.showLoading()
 ...
 if (this.dashboard) {
  ...
  getFacebookCampaginNames(){
    this.facebookLeadService.getFacebookCamapaginNames(this.campaginName,this.digitalPartnerName).subscribe({
      next: (data) => {
