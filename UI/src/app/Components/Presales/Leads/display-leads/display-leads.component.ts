...
import { CallingDialogComponent } from 'src/app/Comman-Components/Dialog/calling-dialog/calling-dialog.component';
import {
  DIGITAL_PARTNER,
  leadPageSizeOptions,
...
import { McubeService } from 'src/app/Services/Presales/MCube/mcube.service';
...
@Component({
  selector: 'app-display-leads',
  templateUrl: './display-leads.component.html',
  styleUrls: ['./display-leads.component.css'],
  encapsulation: ViewEncapsulation.None,
})
export class DisplayLeadsComponent implements OnInit, OnDestroy {
  ...
  //cp user id
  cpUserId: number = 0;
  // Restrict Add Lead  for CP Approval
  ...
  isCTODashboard: boolean = false;
  selectedSubSourcesIds: number[] = [0];
  ...
  // leadRestrictType = 'Add_Lead_Restrict_Roles';
  // leadRestrictRoles: string[] = [];
  isCP: boolean = false;
  isExpried: any = 'N';
  constructor(
    ...
    private mcubeService: McubeService
  ) {}
  private modal: any;
  digitalPartner: any;
  ngOnInit() {
    ...
    if (history.state.navigationId == 1) {
      // alert(this.selectedSubSourcesIds + 'start');
      ...
      // alert(this.selectedSubSourcesIds + 'end');
    }
    ...
    // this.fetchAddLeadRestrictRoles();
  ...
  private initializeState() {
    ...
    // }
    if (history.state.projectId?.length > 0) {
      this.patchStateProjectIds(history.state.projectId);
    }
    ...
    this.isCTODashboard = state.isCTODashboard === true;
    this.isUnassignedLeads = state.isUnassignedLeads === true;
    if (this.isUnassignedLeads === true) {
      this.fetchSubSourcesBySourceIds([8]);
    }
    ...
    this.salesLeads = state.salesLeads === true;
    ...
    this.leadType = history.state.leadType || this.leadType;
    this.isExpried =
  ...
  //     alert(clickedElement !== lastCell);
  //     return clickedElement !== lastCell;
  //   }
  //   return false; // Default to true if structure is unexpected
  // }
  isNotLastColumn(event: MouseEvent): boolean {
    const clickedElement = event.target as HTMLElement;
  ...
  goBack() {
    ...
    console.log(history.state.isCTODashboard);
    console.log(this.isCTODashboard);
    ...
    } else if (role.toLowerCase().includes('digital')) {
      this.isDigitalMarketingDashboard = true;
    }
    if (this.isMemberDashBoard) {
      ...
      this.router.navigate(['layout/ctodashboard'], {
        state: stateData,
      });
    } else {
  ...
  //   this.sourceIds = selectedSource.leadSourceId;
  //   this.fetchSubSources(this.sourceIds);
  //   if (this.isUnassignedLeads) {
  //     this.getDashBoardUnassignedLeadsDetails();
  ...
  getStarCount(lead: any): number {
    ...
    if (lead.isSiteVisitDoneInAnotherProject === 1) {
      starCount += 1;
    }
    return starCount;
  }
  ...
  // fetchSubSources(sourceIds: number[]): void {
  //   // this.selectedSubSource = new LeadSubSource(0, '', 0, '');
  //   this.sourceIds = sourceIds;
  //   this.leadService.fetchLeadSubSources(sourceIds).subscribe({
  //     next: (subSources) => {
  //       this.sourceIds = sourceIds;
  ...
  onSelectionSubSourceChange(subSource: any, event: any) {
    ...
    console.log('sdfghfd' + selectedSubSourceId);
  ...
  searchProject(event: any) {
    console.log('project search : ', event);
  ...
  getLeadTeam() {
    this.commonRefDetailService
      .fetchCommomRefDetailsByType('Lead_Team')
      .subscribe({
        next: (response: any) => {
          this.leadTeam = response;
          //this.hideLoading();
        },
        error: (error) => {
          console.error(error);
  ...
  onLeadTeamSelectionChange(event: any) {
    this.followUpType = event.value;
    if (this.followUpType === 'all') {
      this.followUpType = '';
    }
    if (this.isUnassignedLeads) {
      this.getDashBoardUnassignedLeadsDetails();
  ...
  // fetchAddLeadRestrictRoles() {
  //   this.commonService.getRefDetailsByType(this.leadRestrictType).subscribe({
  //     next: (response: CommonReferenceDetails[]) => {
  //       console.log(response);
  //     },
  //     error: (error) => console.error(error),
  ...
  makeOutboundCall(
    ...
    const dialogRef = this.dialog.open(CallingDialogComponent, {
      disableClose: true,
      width: '340px',
      panelClass: 'calling-dialog-panel',
      data: {
        customerName: customerName || 'Customer',
        phoneNumber: phoneNumber,
      },
    });
    this.mcubeService.makeOutboundCall(phoneNumber, userNumber).subscribe({
      next: (response) => {
        dialogRef.close();
        console.log('Outbound call successful:', response);
      },
      error: (error) => {
        dialogRef.close();
