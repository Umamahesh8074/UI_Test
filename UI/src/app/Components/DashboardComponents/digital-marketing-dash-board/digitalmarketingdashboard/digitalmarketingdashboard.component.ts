...
export interface FoodNode {
  ...
  costPerBooking?: any;
  siteVisitDone?: any;
  costPerSitevisitDone?: any;
}
@Component({
  selector: 'app-digitalmarketingdashboard',
  templateUrl: './digitalmarketingdashboard.component.html',
  styleUrls: ['./digitalmarketingdashboard.component.css'],
})
export class DigitalmarketingdashboardComponent {
  ...
  lostCount: number = 0;
  revisitProposedId = 0;
  isDigitalMarketingDashboard: boolean = true;
  ...
  isDigitalMarketing: boolean = false;
  displayedLeadBudgetColumns: string[] = [
    'sourceName',
    'totalAmonut',
    'leadCount',
  ...
  campaginName:any=''
  campaginNames:any=[]
  campagin = new FormControl({ campaginName: '' }); // Default as an object
  ...
  ngOnInit() {
    ...
    if (user) {
      ...
      if(this.userRole=='DIGITAL_PARTNER'){
       this.digitalPartnerName='FLUTCH'
      }
      this.organizationId = this.user.organizationId;
    }
    if (this.userRole === 'Digital Marketing') {
...
this.digitalPartnerName=history.state.digitalPartner||this.digitalPartnerName
console.log(this.digitalPartnerName)
    this.initForm();
  ...
  navigateToLeads() {
    ...
    let  route = 'layout/presales/digitalleads';
    ...
    const state = {
      ...
      statusId: '',
      isDigitalMarketingDashboard: true,
  ...
  patchDateFilterAndFetchData() {
   ...
   this.campaginName=history.state.camapaginName?history.state.camapaginName:''
  ...
  fetchTotalLeadsCount() {
    // because of the api issue we are using this method to get the total leads count for the user and role id
  ...
  updateLeadSourceChartOptions(res: any) {
    ...
    const requiredSources = ['Digital', 'Social Media'];

    // Initialize series data with zero counts
    const series = new Array(requiredSources.length).fill(0);

    // Map response data to series based on requiredSources
    res.forEach((resp: any) => {
      const index = requiredSources.indexOf(resp.sourceName);
      if (index !== -1) {
    ...
    this.leadSourceChartOptions = {
      ...
      labels: requiredSources, // Use only the required sources for labels
      colors: [
        '#00ACC1', // Digital - Cyan
        '#FFCE56', // Social Media - Yellow
      ],
      responsive: [
        {
          breakpoint: 480,
  ...
  displayFn(campaginName: any): string {
    return campaginName && campaginName.campaginName ? campaginName.campaginName : 'All';
  }
 ...
 this.fetchLeadsDashboardCount()
  ...
  getFacebookCampaginNames(){
    const encodedCampaginName= encodeURIComponent(this.campaginName);
    this.facebookLeadService.getFacebookCamapaginNames(encodedCampaginName,this.digitalPartnerName).subscribe({
      next: (data) => {
        console.log(this.campaginNames)
