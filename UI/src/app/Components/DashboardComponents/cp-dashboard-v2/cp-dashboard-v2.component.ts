...
@Component({
  selector: 'app-cp-dashboard-v2',
  templateUrl: './cp-dashboard-v2.component.html',
  styleUrls: ['./cp-dashboard-v2.component.css'],
})
export class CpDashboardV2Component {
    ...
    if (this.projectId) {
      this.projectService.getProjectById(this.projectId).subscribe({
        next: (response) => {
          ...
          this.project.patchValue(response);
        },
        error: (error) => {
          console.error(error);
        },
  ...
  onProjectSelect(event: any) {
    console.log(event.option.value);
    console.log(event?.option.value?.projectId);
