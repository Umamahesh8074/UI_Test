...
@Component({
  selector: 'app-create-quotations',
  templateUrl: './create-quotations.component.html',
  styleUrls: ['./create-quotations.component.css'],
  encapsulation: ViewEncapsulation.None, // Set ViewEncapsulation to None
})
export class CreateQuotationsComponent
  ...
  implements OnInit
{
  ...
  statePageDataFromCreateQuotation: any;
  ...
  ngOnInit(): void {
    ...
    this.fetchVendors();
    this.fetchProjects();
    this.fetchQuotationDetailsByIndentId();
  ...
  getDataFromState() {
    const {
      ...
      displayPageData,
      displayPage,
      pageDataFromCreateQuotation,
    } = history.state;
    this.indentId = indentId ?? quotation?.indentId ?? this.indentId;
    if (displayPage) {
      this.statePageDataFromCreateQuotation = displayPage;
    }
    if (pageDataFromCreateQuotation) {
      this.pageDataFromCreateQuotation = pageDataFromCreateQuotation;
    }
    if (displayPageData) {
  ...
  addQuotation() {
    this.router.navigate(['layout/procurement/addquotation'], {
      state: {
        indentId: this.indentId,
        isAdding: true,
        status: 'PendingQuotation',
        statePageDataFromCreateQuotation: this.statePageDataFromCreateQuotation,
        displayPage: {
  ...
  fetchQuotationDetailsByIndentId() {
    this.showLoading();
        ...
        next: (response) => {
          ...
          this.router.navigate(['./addquotation'], {
            ...
            state: {
              ...
              routeStatus: 'create',
              isAdding: false,
              quotationType: 'Update',
              status: 'PendingQuotation',
              indentId: this.indentId,
              statePageDataFromCreateQuotation:
                this.statePageDataFromCreateQuotation,
              displayPage: {
                quotationSelectedProject: this.selectedProject,
                quotationSelectedVendor: this.selectedVendor,
                searchedQuotationCode: this.quotationCode,
