...
@Component({
  selector: 'app-non-pending-displayindent',
  templateUrl: './display-non-pending-indent.component.html',
  styleUrls: ['./display-non-pending-indent.component.css'],
  encapsulation: ViewEncapsulation.None,
})
export class DisplayNonPendingIndentComponent implements OnInit {
  ...
  getDataFromState() {
    ...
    if (displayPageData) {
      ...
      this.selectedStatus = displayStatePageData.selectedStatus;
  ...
  editIndent(indentData: any) {
    ...
    this.indentService
      ...
      .subscribe((response) => {
        ...
        this.router.navigate(['layout/procurement/addindent'], {
          state: {
            indent: this.indents,
            indentType: 'Rework',
            status: 'ApprovedIndent',
            displayPage: {
              selectedProject: this.selectedProject,
              searchedIndentCode: this.indentCode,
              searchedStageOwner: this.stageOwnerName,
  ...
  sendForApprove(indent: any) {
    this.indentService
      .moveIndentToWorkFlowAfterRework(indent.indentId, indent.indentType)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
